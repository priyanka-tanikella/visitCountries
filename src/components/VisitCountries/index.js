...
class VisitCountries extends Component {
  ...
  filteredVisitedCountries = () => {
    ...
    const filterCountries = visitedCountriesList.filter(
  ...
  clickVisit = id => {
    this.setState(prevState => ({
      visitedCountriesList: prevState.visitedCountriesList.map(eachCountry => {
  ...
  renderVisitedCountriesView = () => {
    const filteredVisitedCountries = this.filteredVisitedCountries()
    return (
      <ul className="filtered-countries-list">
        {filteredVisitedCountries.map(eachItem => (
          <VisitedCountryItem
            key={eachItem.id}
  ...
  renderNoCountriesVisitedView = () => (
    <div className="no-countries-visited-view-container">
  ...
  render() {
    ...
    const filteredVisitedCountries = this.filteredVisitedCountries()
    const showFilteredCountries = filteredVisitedCountries.length > 0
    console.log(filteredVisitedCountries)
    return (
      <div className="visit-countries-container">
        ...
        {showFilteredCountries
          ...
          : this.renderNoCountriesVisitedView()}
      </div>
    )
  }
}
